...
These core plugins work well with the toolkit (all built into Obsidian):
- **Backlinks** — see which notes link to the current note
- **Graph View** — visualize your learning knowledge graph
- **Templates** — quickly insert the daily note template
- **Daily Notes** — auto-create dated notes (configure the format to `YYYYMMDD` and folder to `Daily Notes`)
- **Tags** — organize by topic
- **Search** — full-text search across all notes

### Daily Notes plugin setup

To have Obsidian auto-create daily notes in the right folder with the right template:

1. Go to **Settings → Core plugins** and enable **Daily notes**
...
Now clicking "Open today's daily note" creates a pre-filled note in the right place.

## Using with Other Editors

This toolkit works with any editor that can open markdown files:

- **VS Code:** Open the `notes/` folder. Install a Markdown preview extension for a nicer editing experience.
- **Typora:** Open individual `.md` files or the `notes/` folder for a clean writing experience.
- **iA Writer:** Add the `notes/` folder to your library.
- **Logseq:** Open `notes/` as a graph (note: Logseq uses a slightly different markdown flavor, but the basics work).
- **Plain text editor:** The files are standard markdown — any editor works.

The key requirement is that your notes live as `.md` files in the `notes/` directory structure. The Claude Code skills read and write to these files directly.
